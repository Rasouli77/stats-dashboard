...
from .models import PeopleCounting, Cam, Merchant, Branch, PeopleCountingHourly
...
def update_or_create_camera_data_hourly(data: list, cam_id, merchant_id, branch_id):
    cam = Cam.objects.get(pk=cam_id)
    merchant = Merchant.objects.get(pk=merchant_id)
    branch = Branch.objects.get(pk=branch_id)
    for item in data:
        PeopleCountingHourly.objects.update_or_create(
            date=item['date'],
            hour=item['hour'],
            cam=cam,
            defaults={
...
def get_custom_date_camera_data_hourly(ip: str, start_date_str: str, end_date_str: str):
    """
    This function takes an IP, a start date and an end date. It returns a dictionary of dates and total entries for a date
    """
    dahua = DahuaRpc(host=ip, username="pc", password="@123456789")
